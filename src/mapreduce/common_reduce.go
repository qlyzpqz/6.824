...
func doReduce(
	...
	dstFile, _ := os.OpenFile(mergeName(jobName, reduceTask), os.O_CREATE | os.O_RDWR, 0666)
	defer dstFile.Close()
	dstEncoder := json.NewEncoder(dstFile)

	for key, values := range kvs {
		value := reduceF(key, values)
		kv.Key = key
		kv.Value = value
		dstEncoder.Encode(kv)
	}
}
